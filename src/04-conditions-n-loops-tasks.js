...
function findFirstSingleChar(str) {
  ...
  let result = null;
  arr.some((el) => {
    const fI = str.indexOf(el);
    const lI = str.lastIndexOf(el);
    if (fI === lI) {
      result = el;
    }
    return result;
...
function getDigitalRoot(num) {
  const brr = num.toString().split('').reduce((acc, curr) => acc + curr * 1, 0);
  if (brr > 9) {
    return getDigitalRoot(brr);
  }
  return brr;
}
...
function getCommonDirectoryPath(pathes) {
  const arr = [];
  pathes.map((elem) => arr.push(elem.split('/')));
  const result = arr.reduce((common, path) => common.filter((segment, index) => path[index]
   === segment));
